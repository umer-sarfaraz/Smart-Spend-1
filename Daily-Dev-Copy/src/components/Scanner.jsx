...
export default function Scanner({ onSave, onOpenManual, stores = [], showToast, nameMap = {}, onLearnNames }) {
  ...
  const handleReviewSave = () => {
    ...
    try {
      confetti({ particleCount: 100, spread: 70, origin: { y: 0.8 }, colors: ['#A99BFA', '#34D399', '#f59e0b', '#f43f5e'] });
    } catch {}
  ...
  const renderCameraView = () => (
    <div>
      ...
      {parsedData && scanCount > 0 && (
        <div style={{display:'flex',alignItems:'center',gap:'8px',padding:'10px 14px',background:'rgba(52, 211, 153,0.06)',border:'1px solid rgba(52, 211, 153,0.2)',borderRadius:'12px',marginBottom:'10px',fontSize:'0.75rem',color:'#34d399'}}>
      ...
      <div style={{marginBottom:'12px'}}>
        <div style={{fontSize:'0.65rem',fontWeight:700,color:'#5A6270',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:'8px'}}>
          ...
          {STORE_TYPES.map(s => (
            <button
              ...
              style={{
                ...
                border:`1px solid ${storeType === s.id ? 'rgba(169, 155, 250,0.6)' : 'rgba(255,255,255,0.08)'}`,
                background: storeType === s.id ? 'rgba(169, 155, 250,0.18)' : 'rgba(255,255,255,0.03)',
                color: storeType === s.id ? '#CDC5FC' : '#5A6270',
      ...
      <div className="scan-hero" style={{height:300}}>
        {!cameraError ? (
          ...
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'10px',padding:'20px'}}>
            <Camera size={44} style={{color:'#A99BFA'}} />
            <p style={{fontSize:'0.8rem',color:'#fff',fontWeight:700,textAlign:'center'}}>Camera Permissions Blocked</p>
            <p style={{fontSize:'0.7rem',color:'#8A93A0',lineHeight:'1.4',textAlign:'center',maxWidth:'240px'}}>
  ...
  const renderProcessingView = () => (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'60px 0',gap:'16px'}}>
      <div className="loading-ring" style={{width:'44px',height:'44px',borderTopColor:'#A99BFA'}} />
      <div style={{textAlign:'center'}}>
        <p style={{fontWeight:700,fontSize:'0.95rem'}}>
          {scanCount > 0 ? `Reading Section ${scanCount + 1}...` : 'Extracting Bill Details'}
        </p>
        <p style={{fontSize:'0.75rem',color:'#5A6270',marginTop:'4px'}}>{statusMessage}</p>
      </div>
    </div>
  );
  ...
  const renderReviewView = () => {
    ...
    return (
      <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
        ...
        <div className="review-header">
          ...
          <div style={{flex:1}}>
            <h3 style={{fontFamily:'var(--font-title)',fontSize:'1.05rem',fontWeight:800}}>
              Review Receipt
              {scanCount > 0 && (
                <span style={{fontSize:'0.68rem',background:'rgba(169, 155, 250,0.15)',color:'#CDC5FC',padding:'2px 8px',borderRadius:'20px',fontWeight:700,marginLeft:8}}>
        ...
        <div style={{display:'flex',alignItems:'flex-start',gap:'6px',padding:'7px 12px',background:usedAI?'rgba(52, 211, 153,0.08)':'rgba(245,158,11,0.08)',border:`1px solid ${usedAI?'rgba(52, 211, 153,0.25)':'rgba(245,158,11,0.25)'}`,borderRadius:'10px',fontSize:'0.72rem',fontWeight:700,color:usedAI?'#34d399':'#f59e0b'}}>
            ...
            <div key={idx} style={{position:'relative'}}>
              <div className="review-item-row">
                    ...
                    <div className="cat-picker-popup" onClick={e => e.stopPropagation()} onWheel={e => e.stopPropagation()}>
                      {Object.entries(CATEGORIES).map(([key, val]) => (
                        ...
                        >
                          ...
                          <span style={{fontSize:'0.58rem',fontWeight:700,color:'#8A93A0',lineHeight:1.2,textAlign:'center'}}>{val.label}</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <button
                  ...
                  style={{background:'rgba(169, 155, 250,0.06)', border:'1px solid rgba(169, 155, 250,0.2)', color:'#CDC5FC', padding:'8px', borderRadius:'8px', cursor:'pointer', display:'flex', alignItems:'center', flexShrink:0}}
                >
                  <BookOpen size={14} />
                </button>
                ...
                <button
                  ...
                  style={{background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.2)',color:'#fb7185',padding:'8px',borderRadius:'8px',cursor:'pointer',display:'flex',alignItems:'center',flexShrink:0}}
                >
                  <Trash2 size={14} />
                </button>
              </div>
        ...
        {totalDiff !== null && (
          totalMatches ? (
            <div style={{display:'flex',alignItems:'center',gap:'8px',padding:'9px 12px',background:'rgba(52, 211, 153,0.08)',border:'1px solid rgba(52, 211, 153,0.25)',borderRadius:'10px',fontSize:'0.74rem',fontWeight:700,color:'#34d399'}}>
              <Check size={14} style={{flexShrink:0}} />
              <span>Items match the receipt's printed {printedLabel} (${printed.toFixed(2)}) — extraction verified</span>
            </div>
          ) : (
            <div style={{display:'flex',alignItems:'flex-start',gap:'8px',padding:'9px 12px',background:'rgba(245,158,11,0.08)',border:'1px solid rgba(245,158,11,0.3)',borderRadius:'10px',fontSize:'0.74rem',color:'#f59e0b',lineHeight:1.45}}>
        ...
        <div className="review-total-card">
          <span style={{fontSize:'0.82rem',fontWeight:700,color:'#8A93A0'}}>TOTAL</span>
          <span style={{fontFamily:'var(--font-title)',fontSize:'1.4rem',fontWeight:800,color:'#34D399'}}>
            ${total.toFixed(2)}
          </span>
        </div>
        ...
        <button
          ...
          style={{background:'none',border:'none',color:'#5A6270',fontSize:'0.72rem',cursor:'pointer',textAlign:'center',paddingBottom:'4px'}}
        >
          Start over
        </button>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:'4px'}} onClick={() => { openPicker !== null && setOpenPicker(null); }}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'14px'}}>
        <Sparkles size={18} style={{color:'#A99BFA'}} />
