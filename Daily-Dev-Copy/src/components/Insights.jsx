...
const ChartTip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: '#141921', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '8px 12px', fontSize: '0.75rem', boxShadow: '0 8px 20px rgba(0,0,0,0.5)' }}>
      <div style={{ color: '#8A93A0', marginBottom: 2 }}>{label}</div>
...
export default function Insights({ expenses = [], budget = 0, catBudgets = {} }) {
  ...
  const kpiCard = (label, value, sub) => (
    <div className="glass-card" style={{ flex: 1, minWidth: 0, padding: '12px 14px' }}>
      <p style={{ fontSize: '0.62rem', fontWeight: 700, color: '#5A6270', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</p>
      ...
      {sub && <p style={{ fontSize: '0.62rem', color: '#8A93A0', marginTop: 1 }}>{sub}</p>}
    </div>
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        {PERIODS.map(p => (
          <button
            ...
            style={{
              ...
              border: `1px solid ${period === p.id ? 'rgba(169, 155, 250,0.6)' : 'rgba(255,255,255,0.08)'}`,
              background: period === p.id ? 'rgba(169, 155, 250,0.18)' : 'rgba(255,255,255,0.03)',
              color: period === p.id ? '#CDC5FC' : '#5A6270',
      ...
      {empty ? (
        <div className="glass-card">
          <EmptyState
            icon={expenses.length === 0 ? <Receipt size={22} /> : <CalendarDays size={22} />}
            title={expenses.length === 0 ? 'No expenses yet' : 'Nothing this period'}
            message={expenses.length === 0
              ? 'Scan a receipt or add an expense, and your charts will light up here.'
              : 'No expenses in the selected range — try another period above.'}
          />
        </div>
      ) : (
        <>
          ...
          {highlights.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {highlights.map((h, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '11px 14px', background: `${h.color}10`, border: `1px solid ${h.color}35`, borderRadius: '14px', fontSize: '0.78rem', lineHeight: 1.5, color: '#F4F6F8' }}>
          ...
          {Object.keys(catBudgets).length > 0 && (() => {
            ...
            return (
              <div className="glass-card">
                <h3 className="section-title">🎯 Category Budgets <span style={{ fontSize: '0.6rem', color: '#5A6270', fontWeight: 700 }}>· this month</span></h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: 4 }}>
                  {Object.entries(catBudgets).map(([key, limit]) => {
                    ...
                    const color = pct < 70 ? '#34D399' : pct < 100 ? '#f59e0b' : '#f43f5e';
                    return (
                      <div key={key}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: 4 }}>
                          <span style={{ color: '#8A93A0', fontWeight: 600 }}>{CATEGORIES[key]?.icon} {CATEGORIES[key]?.label || key}</span>
                          <span style={{ fontWeight: 700, color: spent > limit ? '#f43f5e' : '#8A93A0' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '46%', height: 170, position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                  <span style={{ fontSize: '0.58rem', fontWeight: 700, color: '#5A6270' }}>TOTAL</span>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: 170, overflowY: 'auto', paddingRight: 4 }}>
                {donutData.slice(0, 6).map(d => (
                  <div key={d.key} style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '0.72rem' }}>
                    ...
                    <span style={{ flex: 1, color: '#8A93A0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.icon} {d.name}</span>
                    <span style={{ fontWeight: 700, color: '#8A93A0', flexShrink: 0 }}>{d.pct}%</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ height: 150 }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailyData} margin={{ top: 6, right: 0, left: -22, bottom: 0 }}>
                  <XAxis dataKey="label" tick={{ fontSize: 10, fill: '#5A6270' }} axisLine={false} tickLine={false} interval={1} />
                  <YAxis tick={{ fontSize: 10, fill: '#5A6270' }} axisLine={false} tickLine={false} tickFormatter={fmtShort} />
                  <Tooltip content={<ChartTip />} cursor={{ fill: 'rgba(255,255,255,0.04)' }} labelFormatter={(l, p) => p?.[0]?.payload?.full || l} />
                  <Bar dataKey="amt" fill="#A99BFA" radius={[5, 5, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div style={{ height: 160 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={monthlyData} margin={{ top: 6, right: 4, left: -22, bottom: 0 }}>
                  ...
                  <XAxis dataKey="label" tick={{ fontSize: 10, fill: '#5A6270' }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 10, fill: '#5A6270' }} axisLine={false} tickLine={false} tickFormatter={fmtShort} />
                  <Tooltip content={<ChartTip />} />
                  <Area type="monotone" dataKey="amt" stroke="#34D399" strokeWidth={2.5} fill="url(#trendFill)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="glass-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: 4 }}>
                {merchants.map(([name, amt], i) => (
                  <div key={name}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: 4 }}>
                      <span style={{ color: '#8A93A0', fontWeight: 600 }}>{i + 1}. {name}</span>
                      <span style={{ fontWeight: 800, color: '#fff' }}>{fmtMoney(amt)}</span>
                    </div>
                    <div style={{ height: 6, borderRadius: 4, background: 'rgba(255,255,255,0.05)' }}>
                      <div style={{ height: '100%', borderRadius: 4, width: `${Math.max(4, amt / merchMax * 100)}%`, background: 'linear-gradient(90deg, #A99BFA, #A99BFA)' }} />
                    </div>
                  </div>
                ))}
              </div>
