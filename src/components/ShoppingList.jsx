...
// ── Catalog: all browsable items grouped by category ─────────────────────────
const CATALOG = {
  vegetables: [
    ...
    'Peas', 'Peppers', 'Potatoes', 'Spinach', 'Sweet Potatoes', 'Tomatoes',
    'Zucchini',
  ],
  fruits: [
  ...
  other: [
    'Batteries', 'Candles', 'Dog Food', 'Cat Food', 'First Aid Kit',
    'Vitamins', 'Zip Lock Bags',
  ],
};
...
export default function ShoppingList({
  ...
  const addCustomItem = () => {
    ...
    onAddItem({ name: customName.trim(), checked: false, store: customStore, category: customCategory });
  ...
  const saveEdit = () => {
    ...
    onUpdateItem(editIdx, { name: editName.trim(), store: editStore, category: editCat });
                  ...
                  {!showCustomForm ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button onClick={() => { setShowCustomForm(false); setCustomName(''); }} className="outline-btn" style={{ width: 'auto', padding: '12px' }}><X size={16} /></button>
                      </div>
                    </div>
                  )}
                </div>
